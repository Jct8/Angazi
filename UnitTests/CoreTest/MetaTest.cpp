...
namespace CoreTest
{
	...
	public:
		...
		{
			auto carType = Meta::GetMetaType<Tesla>();
			Assert::IsTrue(carType->GetCategory() == Meta::MetaType::Category::Class);
			Assert::AreEqual(carType->GetName(), "Tesla");
			Assert::AreEqual(carType->GetSize(), sizeof(Tesla));
